...
new class extends Component {
    ...
    {
        return [
            'listeningParties' => ListeningParty::with('episode.podcast')->where('is_active', true)->whereNotNull('end_time')->orderBy('start_time', 'asc')->get(),
        ];
    }
}; ?>
...
<div class="flex flex-col min-h-screen pt-8 bg-emerald-50">
    ...
    <div class="my-20">
        <div class="max-w-lg mx-auto">
            <h3 class="font-serif mb-4 font-bold text-[0.9rem]">Upcoming Listening Parties</h3>

            <div class="bg-white rounded-lg shadow-lg">
                @if ($listeningParties->isEmpty())
                    <div class="flex justify-center items-center py-6 font-serif text-sm">No awwdio listening parties
                        started yet... 😔</div>
                @else
                    @foreach ($listeningParties as $listeningParty)
                        <div wire:key="{{ $listeningParty->id }}">
                            <a href="{{ route('parties.show', $listeningParty) }}">
                                <div
                                    class="flex items-center justify-between p-4 border-gray-200 border-b hover:bg-gray-50 transition-all duration-150 ease-in-out">
                                    <div class="flex items-center space-x-4">
                                        <div class="flex-shrink-0">
                                            <x-avatar src="{{ $listeningParty->podcast->artwork_url }}" size="xl"
                                                rounded="sm" alt="Podcast Artwork" />
                                        </div>
                                        <div class="flex-1 min-w-0">
                                            <p class="text-[0.9rem] font-semibold text-slate-900 truncate max-w-xs">
                                                {{ $listeningParty->name }}
                                            </p>
                                            <div class="mt-[0.8px] mx-auto">
                                                <p class="text-sm text-slate-600 truncate max-w-xs">
                                                    {{ $listeningParty->episode->title }}
                                                </p>
                                                <p
                                                    class="text-[0.7rem] text-slate-400 uppercase tracking-tighter truncate max-w-xs">
                                                    {{ $listeningParty->podcast->title }}
                                                </p>
                                            </div>
                                            <div class="text-xs text-slate-600 mt-1" x-data="{
                                                ...
                                                countdownText: '',
                                                isLive: '{{ $listeningParty->is_active && $listeningParty->start_time->isPast() }}',
                                                updateCountdown() {
